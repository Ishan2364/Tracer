# Eval v2 — 15 Test Cases (Full Feature Coverage)

Covers everything built since the original 12-case set: grounded Q&A, multi-episode
comparison (including partial coverage and ambiguous title resolution), broad/refusal,
recommendation, nonexistent-episode handling, time-range-scoped retrieval, chitchat +
its Gate-2 safety net, catalogue/inventory consistency, and multi-turn memory (both
context-continuation and clean topic-switch).

**Status: RUN.** Results below are real — executed via `src/eval/run_eval_v2.py`,
raw output preserved untouched in `eval/results_v2/raw/case_NN.json`. Every citation
...
**Final tally: 13 Pass / 2 Partial / 0 Fail** (out of 15) — Case 9's fail was a real bug and has been fixed and re-verified (see Case 9 below); the 2 Partials are citation-precision refinements, not correctness failures, and remain open by choice.
...
**Matching:** First citation matches exactly. Second citation is a genuine overshoot.
**Pass/Fail:** **Partial**
**Failure analysis:** Confirmed by direct calculation: `retrieve_scoped_timerange()` filters on a chunk's `start` timestamp only (`$gte`/`$lte` on `start`), not on `end`. A chunk that starts at 27:51 (inside the window) but runs to 29:21 (well outside it) still qualifies and gets cited. The model itself seemed to sense the mismatch — it hedged with a malformed "28:??" in the free text rather than stating the chunk's real end time — which is itself a minor presentation glitch, but the structured citation (what the product's trust mechanism actually relies on) still reports the full 27:51–29:21 range. Fix: filter should also require the chunk's `end` to not exceed the window by more than a small tolerance, or fall back to utterance-level filtering for tighter enforcement.
